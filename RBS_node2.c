...
uint32 SampleApp_ConvertToUint(void);
...
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
  switch ( pkt->clusterId )
  {
    case SAMPLEAPP_PERIODIC_CLUSTERID:
      myTime = osal_getClock();
      SampleApp_ConvertToUChar(myTime);
      //SampleApp_SendClockMessage();
      HalUARTWrite(0,"My clock\n",9);
      HalUARTWrite(0, converted,10);
      HalUARTWrite(0,"\n",1);
      break;

    case SAMPLEAPP_FLASH_CLUSTERID:
      for (int i = 0; i < 10; i++)
        received[i] = pkt->cmd.Data[i];
      othersTime = SampleApp_ConvertToUint();
      othersTime++;
      HalUARTWrite(0,"Received clock\n",15);
      HalUARTWrite(0, &pkt->cmd.Data[0],10);
      HalUARTWrite(0,"\n",1);
      currentTime = osal_getClock();
      osal_setClock(currentTime + othersTime - myTime);
      currentTime = osal_getClock();
      SampleApp_ConvertToUChar(currentTime);
      HalUARTWrite(0,"Set clock\n",10);
      HalUARTWrite(0, converted,10);
      HalUARTWrite(0,"\n",1);
      break;
  }
...
uint32 SampleApp_ConvertToUint(void)
{
  ...
  for (int i = 10; i > 0; i--)
  {
    ...
    else if (received[i - 1] == '2')
      seconds += 2 * pow(10, 10 - i);
    else if (received[i - 1] == '3')
      seconds += 3 * pow(10, 10 - i);
    else if (received[i - 1] == '4')
      seconds += 4 * pow(10, 10 - i);
    else if (received[i - 1] == '5')
      seconds += 5 * pow(10, 10 - i);
    else if (received[i - 1] == '6')
      seconds += 6 * pow(10, 10 - i);
    else if (received[i - 1] == '7')
      seconds += 7 * pow(10, 10 - i);
    else if (received[i - 1] == '8')
      seconds += 8 * pow(10, 10 - i);
    else if (received[i - 1] == '9')
